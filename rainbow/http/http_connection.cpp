...
HttpConnection::~HttpConnection() {
    RAINBOW_LOG_ERROR(g_logger) << "HttpConnection::~HttpConnection()";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if (client_parser.chunked) {
        ...
        do {
            ...
            len -= 2;
            RAINBOW_LOG_INFO(g_logger) << "connect_len = " << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = rainbow::GetCurrentMS();
    ...
    if (!ptr) {
        ...
        if (!addr) {
            RAINBOW_LOG_ERROR(g_logger) << "get addr fail: " << *addr;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if (!sock) {
            RAINBOW_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if (!sock->connect(addr)) {
            RAINBOW_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if (!ptr->isConnected()
            || (ptr->m_createTime + pool->m_maxAliveTime) >= rainbow::GetCurrentMS()
